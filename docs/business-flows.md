...
#### 业务流程（最终采用版本 — 与 backend-business-logic.md §4.5 一致）
```
需求计划(APPROVED) → 采购计划(DRAFT → SUBMITTED → APPROVED)
  ↓ [下推]
...
合格品                    不合格品
  ↓                        ↓
入库登卡(库存增加)         不良品台账 → 采购退供 / 报废处置
```

> ⚠️ 与旧版差异：旧版写"到货确认 → 入库单 → 质检"，新版明确为"到货确认 → 入库单(待质检) → 质检 → 合格品登卡入库"。库存生效点在登卡，不在审批。

#### 状态机
```
DRAFT → SUBMITTED → APPROVED → 下推/登卡
                  → REJECTED → DRAFT
```
...
| 字段 | 来源系统列名 | 说明 |
|---|---|---|
| approvalStatus | 审批状态 | 原系统数字状态: 5=未提交(DRAFT), 9=已通过(APPROVED)。SUBMITTED/REJECTED 对应数字值 ⚠️ TODO 待确认 |
| inspectionNo | 质检单号 | |
...
#### 业务流程（最终采用版本 — 与 backend-business-logic.md §4.5 一致）
```
到货确认 → 入库单(待质检, DRAFT) → 提交审批 → 质检
  ↓                        ↓
合格品                    不合格品
  ↓                        ↓
入库登卡(库存增加)         不良品台账 → 退供/报废
```

> ⚠️ 库存生效点 = 入库登卡，非审批通过。审批通过后允许登卡，登卡后库存才增加。
...
> ⚠️ 编码前缀必须以原系统实际数据为准。以下为「原系统编码前缀映射表（待验证）」：

| 实体 | 本系统当前前缀 | 原系统截图/接口编码样例 | 确认状态 |
|------|---------------|------------------------|----------|
| 客户 | CUS | `KH202604140001` | ⚠️ 冲突：原系统用 KH，本系统用 CUS |
| 供应商 | SUP | `GYS...` (待确认) | ⚠️ 疑似冲突 |
| 物料 | MAT | `MAT...` | 待原系统验证 |
| 合同 | — | — | 待确认 |
| 报价单 | QTE | — | 待原系统验证 |
| 销售订单 | SO | — | 待原系统验证 |
...
**格式规则：** 各类单据编码格式: 前缀+日期+流水号（日期取 yyyyMM 或 yyyyMMdd 取决于业务密度）。物料: 外部编码 + 内部编码双编码体系。合同: 按合同类型分别编码。
...
### 采购链路 (需求→计划→订单→到货→入库→质检→登卡→退供)

> 此版本为最终采用版本，与 backend-business-logic.md §4.5 一致。旧版中「先质检再入库」与「先入库再质检」的矛盾已统一。
...
```
  ...
  到货确认
       ↓ [自动生成]
  入库单(待质检, DRAFT)            ← 物料进入待检状态（不增加可用库存）
       ↓ [提交→审批]
  质检单(DRAFT) ──→ SUBMITTED ──→ APPROVED
       ↓                              ↓
  [合格品]                        [不合格品]
...
**入库单 "登卡" 机制（库存生效点）:**
- `登卡`: 确认入库, 创建/更新库存卡片记录 → **库存正式增加**
- `撤销登卡`: 取消入库, 库存卡片失效 → 库存回滚到登卡前状态
- **审批通过 ≠ 库存生效**：审批通过后允许登卡，但库存变化发生在登卡执行时
- 入库单分为: 合格品入库 / 不良品入库 (分仓库)
- 登卡后入库单 businessStatus → CLOSED
...
### 仓储链路
```
  入库: 到货确认 → 入库单(待质检) → 质检 → 合格品登卡 → 库存增加
        不合格品 → 不良品台账 → 退供/报废
  出库: 出货通知 → 出库单 → 下架登卡 → 库存减少
  调拨: 调出单(OUT) + 调入单(IN) → 登卡 → 库存转移
  盘点: 盘点单 → [盘盈/盘亏] → 调整单审核 → 登卡 → 库存修正
  报废: 报废申请 → 报废处置 → 登卡 → 库存减少 → 报废台账
  借还: 借出单 → 登卡(锁定) → [归还] → 归还登卡 → 库存恢复
```

> 所有库存变动均以“登卡”为生效点，审批通过只是允许执行登卡操作。
