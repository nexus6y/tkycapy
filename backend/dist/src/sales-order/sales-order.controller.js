...
let SalesOrderController = class SalesOrderController {
    ...
    async findAll(status, bizStatus, code, name, page = 1, pageSize = 30) {
        const tenantId = await this.tid();
        const where = { tenantId };
        if (status)
            where.approvalStatus = status;
        if (bizStatus)
            where.businessStatus = bizStatus;
        if (code)
    ...
    async create(dto) { const tenantId = await this.tid(); return this.prisma.salesOrder.create({ data: { ...dto, tenantId } }); }
    async update(id, dto) { return this.prisma.salesOrder.update({ where: { id }, data: dto }); }
    async remove(id) { await this.prisma.salesOrder.delete({ where: { id } }); return { message: '删除成功' }; }
    async submit(id) { return this.prisma.salesOrder.update({ where: { id }, data: { approvalStatus: 'SUBMITTED' } }); }
};
exports.SalesOrderController = SalesOrderController;
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)('status')),
    __param(1, (0, common_1.Query)('bizStatus')),
...
__decorate([
    ...
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], SalesOrderController.prototype, "submit", null);
