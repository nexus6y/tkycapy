...
const KEYS = ['returnNo','shipmentId','shipmentNo','customerId','customerCode','customerName','returnDate','totalQuantity','totalAmount','returnReason','approvalStatus','businessStatus','remark','tenantId'];
...
export class SalesReturnController {
  ...
  @Get(':id') async findOne(@Param('id') id: string) { return this.prisma.salesReturn.findUniqueOrThrow({ where: { id } }); }
  @Post() async create(@Body() dto: any) {
    try {
      const tenantId = await this.tid();
      if (!dto.returnNo) dto.returnNo = await this.codeGen.generate('SRT', 'salesReturn', 'returnNo');
      return await this.prisma.salesReturn.create({ data: clean(dto, tenantId) });
    }
    catch (e: any) { if (e.code === 'P2002') throw new HttpException('退货单号已存在', HttpStatus.BAD_REQUEST); throw e; }
  }
  @Put(':id') async update(@Param('id') id: string, @Body() dto: any) { const tenantId = await this.tid(); return this.prisma.salesReturn.update({ where: { id }, data: clean(dto, tenantId) }); }
  ...
  @Put(':id/approve') async approve(@Param('id') id: string) { return this.prisma.salesReturn.update({ where: { id }, data: { approvalStatus: 'APPROVED' } as any }); }
}
