...
const LABELS: Record<string, string> = { '/':'工作台','/material':'物料档案','/material-category':'物料分类','/material-param':'物料参数','/material-approval':'物料审批','/project':'项目维护','/project/query':'项目查询','/contract':'合同维护','/contract/params':'合同参数','/contract/query':'合同查询','/sales/customer':'客户档案','/sales/params':'销售参数','/sales/quotation':'报价单维护','/sales/pre-order':'分劈单维护','/sales/order':'销售订单维护','/sales/shipment':'销售出货维护','/sales/return':'销售退货维护','/sales/trace':'销售执行追溯' };

export default function TagsView() {
  const router = useRouter(); const pathname = usePathname();
  const [tags, setTags] = useState<Tag[]>([]);
  const getL = (p: string) => LABELS[p] || p.split('/').pop() || p;
