...
const save=async()=>{if(lines.length>0){f.totalQuantity=String(lines.reduce((s,ln)=>s+Number(ln.quantity||0),0));}await api.put('/demand-plans/'+id,{...f,lines});router.push('/ops/demand-plan');};
...
<FormField label="数量"><Input type="number" className={FI} value={f.totalQuantity||''} disabled/></FormField>
