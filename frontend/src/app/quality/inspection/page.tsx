...
export default function InspectionPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><span className="text-[12px] text-muted-foreground">{i.businessStatus}</span></ErpTd><ErpTd><ErpLink onClick={()=>router.push(`/quality/inspection/${i.id}/edit`)}>{i.inspectionNo}</ErpLink></ErpTd><ErpTd className="text-muted-foreground">{i.sourceType||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.sourceNo||'-'}</ErpTd><ErpTd>{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity||'-'}</ErpTd><ErpTd className="text-success">{i.qualifiedQty||'0'}</ErpTd><ErpTd className="text-destructive">{i.unqualifiedQty||'0'}</ErpTd><ErpTd className="text-muted-foreground">{i.inspector||'-'}</ErpTd><ErpTd>{i.result||'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction><ErpActionBtn onClick={()=>router.push(`/quality/inspection/${i.id}/edit`)}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>{i.approvalStatus==='DRAFT'&&<button onClick={()=>{api.put(`/inspections/${i.id}/submit`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'提交失败','error'));}} className="text-primary text-[13px] hover:underline">提交</button>}{i.approvalStatus==='SUBMITTED'&&<button onClick={()=>{api.put(`/inspections/${i.id}/approve`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'审核失败','error'));}} className="text-green-600 text-[13px] hover:underline">审核/生成入库</button>}<ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
