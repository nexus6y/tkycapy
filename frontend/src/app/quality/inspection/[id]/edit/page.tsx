'use client';import { useEffect, useState } from 'react';import { useRouter, useParams } from 'next/navigation';import api from '@/lib/api';import { Input } from '@/components/ui/input';import { Select,SelectContent,SelectItem,SelectTrigger,SelectValue } from '@/components/ui/select';import { RadioGroup,RadioGroupItem } from '@/components/ui/radio-group';import { toast } from '@/components/ui/toast';
...
const save=async()=>{try{await api.put('/inspections/'+id,f);router.push('/quality/inspection');}catch(e:any){toast(e.response?.data?.message||'保存失败','error');}};
...
<FormField label="来源类型"><RadioGroup value={f.sourceType||'采购单'} onValueChange={(v:any)=>setF({...f,sourceType:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="采购单" id="ie-po"/><label htmlFor="ie-po" className="text-[13px]">采购单</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="生产退料" id="ie-pr"/><label htmlFor="ie-pr" className="text-[13px]">生产退料</label></div></RadioGroup></FormField>
