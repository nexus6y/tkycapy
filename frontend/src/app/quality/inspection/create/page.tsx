...
export default function InspectionCreatePage() {
  ...
  const save=async()=>{try{await api.post('/inspections',{...f,quantity:+f.quantity});router.push('/quality/inspection');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
  return (<FormLayout title="新增质检单" onSave={save} sections={SECS} activeSection="basic">
    <FormSection id="basic" title="质检单信息"><FormGrid>
      ...
      <FormField label="来源类型" required>
        <RadioGroup value={f.sourceType} onValueChange={v=>setF({...f,sourceType:v})} className="flex gap-4 pt-1.5">
