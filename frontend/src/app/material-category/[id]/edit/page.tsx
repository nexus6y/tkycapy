...
const save=async()=>{try{await api.put('/material-categories/'+id,f);router.push('/material-category');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
...
<FormField label="上级分类"><Select value={f.parentId||''} onValueChange={v=>setF({...f,parentId:v==='NONE'?'':v})}><SelectTrigger className={FI}><SelectValue placeholder="无"/></SelectTrigger><SelectContent><SelectItem value="NONE">无</SelectItem>{cats.filter((c:any)=>c.id!==id).map((c:any)=><SelectItem key={c.id} value={c.id}>{c.code} {c.name}</SelectItem>)}</SelectContent></Select></FormField>
<FormField label="排序"><Input type="number" className={FI} value={f.sortOrder} onChange={e=>setF({...f,sortOrder:+e.target.value})}/></FormField>
<FormField label="状态"><RadioGroup value={f.status} onValueChange={v=>setF({...f,status:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="ACTIVE" id="me-a"/><label htmlFor="me-a" className="text-[13px]">启用</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="INACTIVE" id="me-i"/><label htmlFor="me-i" className="text-[13px]">停用</label></div></RadioGroup></FormField>
