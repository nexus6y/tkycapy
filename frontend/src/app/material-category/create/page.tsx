...
export default function MaterialCategoryCreate() {
  ...
  useEffect(()=>{api.get('/common/next-code',{params:{entity:'materialCategory'}}).then(r=>setF((prev:any)=>({...prev,code:r.data.code})));api.get('/material-categories',{params:{pageSize:200}}).then(r=>setCats(r.data.items));},[]);
  const save=async()=>{
    try{
      const payload:any={...f};
  ...
  return (<FormLayout title="新增物料分类" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b">
    <FormSection id="b" title="基本信息"><FormGrid>
      <FormField label="分类编码" required><Input className={FI} value={f.code} readOnly disabled/></FormField>
      <FormField label="分类名称" required><Input className={FI} value={f.name} onChange={e=>setF({...f,name:e.target.value})} data-testid="cat-name-input"/></FormField>
