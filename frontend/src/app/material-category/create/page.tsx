...
export default function MaterialCategoryCreate() {
  ...
  const save=async()=>{try{await api.post('/material-categories',f);router.push('/material-category');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
  return (<FormLayout title="新增物料分类" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b">
    <FormSection id="b" title="基本信息"><FormGrid>
      ...
      <FormField label="上级分类"><Select value={f.parentId} onValueChange={v=>setF({...f,parentId:v==='NONE'?'':v})}><SelectTrigger className={FI}><SelectValue placeholder="无"/></SelectTrigger><SelectContent><SelectItem value="NONE">无</SelectItem>{cats.map(c=><SelectItem key={c.id} value={c.id}>{c.code} {c.name}</SelectItem>)}</SelectContent></Select></FormField>
      <FormField label="排序"><Input type="number" className={FI} value={f.sortOrder} onChange={e=>setF({...f,sortOrder:+e.target.value})}/></FormField>
      <FormField label="状态"><RadioGroup value={f.status} onValueChange={v=>setF({...f,status:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="ACTIVE" id="mc-a"/><label htmlFor="mc-a" className="text-[13px]">启用</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="INACTIVE" id="mc-i"/><label htmlFor="mc-i" className="text-[13px]">停用</label></div></RadioGroup></FormField>
