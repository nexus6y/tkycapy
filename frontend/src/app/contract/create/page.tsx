...
useEffect(()=>{api.get('/common/next-code',{params:{entity:'contract'}}).then(r=>setF((prev:any)=>({...prev,code:r.data.code})));},[]);const [f,setF]=useState({code:'',name:'',type:'销售合同',customerName:'',supplierName:'',totalAmount:'',startDate:'',endDate:''});
const save=async()=>{try{await api.post('/contracts',f);router.push('/contract');}catch(e:any){toast(e.response?.data?.message||'保存失败','error');}};
...
<FormField label="合同类型" required><Select value={f.type} onValueChange={(v:any)=>setF({...f,type:v})}><SelectTrigger className={FI}><SelectValue/></SelectTrigger><SelectContent><SelectItem value="销售合同">销售合同</SelectItem><SelectItem value="采购合同">采购合同</SelectItem></SelectContent></Select></FormField>
