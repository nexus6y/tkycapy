...
export default function RoleEdit(){const router=useRouter();const {id}=useParams();const [f,setF]=useState({code:'',name:'',description:'',sortOrder:0,status:'ACTIVE'});
useEffect(()=>{api.get('/roles-mgmt/'+id).then(r=>setF(r.data));},[id]);
const save=async()=>{await api.put('/roles-mgmt/'+id,f);router.push('/system/role');};
return(<FormLayout title="修改角色" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b"><FormSection id="b" title="基本信息"><FormGrid>
