...
<FormField label="类型" required><Select value={f.type} onValueChange={v=>setF({...f,type:v})}><SelectTrigger className={FI}><SelectValue/></SelectTrigger><SelectContent><SelectItem value="API">API</SelectItem><SelectItem value="MENU">MENU</SelectItem></SelectContent></Select></FormField>
<FormField label="所属角色"><Select value={f.roleId} onValueChange={v=>setF({...f,roleId:v})}><SelectTrigger className={FI}><SelectValue placeholder="请选择"/></SelectTrigger><SelectContent>{roles.map(r=><SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>)}</SelectContent></Select></FormField>
