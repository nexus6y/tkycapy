...
export default function DeptEdit(){const router=useRouter();const {id}=useParams();const [f,setF]=useState({code:'',name:'',sortOrder:0,status:'ACTIVE'});
useEffect(()=>{api.get('/departments/'+id).then(r=>setF(r.data));},[id]);
const save=async()=>{await api.put('/departments/'+id,f);router.push('/system/dept');};
return(<FormLayout title="修改部门" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b"><FormSection id="b" title="基本信息"><FormGrid>
