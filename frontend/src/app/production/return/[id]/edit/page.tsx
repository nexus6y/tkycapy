...
useEffect(()=>{api.get('/return-orders/'+id).then(r=>{const d=r.data;setF({...d,endDate:d.endDate?d.endDate.split('T')[0]:''});});},[id]);
const save=async()=>{await api.put('/return-orders/'+id,f);router.push('/production/return');};
...
<FormField label="订单名称" required><Input className={FI} value={f.orderName} onChange={e=>setF({...f,orderName:e.target.value})} data-testid="return-ordername-input"/></FormField>
