...
interface Item { id:string;orderNo:string;orderName:string;materialName:string|null;quantity:string;departmentName:string|null;startDate:string|null;endDate:string|null;approvalStatus:string;businessStatus:string;createdAt:string; }
...
export default function CompleteAuditPage() {
  ...
  const fetch=useCallback(async()=>{
    const p:any={page:pg,pageSize:ps}; if(s.code)p.code=s.code; if(s.name)p.name=s.name; if(s.status)p.status=s.status; p.biz='PENDING_STOCK';
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh>订单单号</ErpTh><ErpTh>订单名称</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>部门</ErpTh><ErpTh>开始日</ErpTh><ErpTh>结束日</ErpTh><ErpTh>业务状态</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><ErpLink>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.orderName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.departmentName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.startDate?new Date(i.startDate).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.endDate?new Date(i.endDate).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd><ErpApproval status={i.businessStatus} labels={{PENDING_STOCK:'待入库',COMPLETED:'已完成'}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd>{i.approvalStatus==='SUBMITTED'?<div className="flex gap-1"><button onClick={()=>audit(i.id,'APPROVED')} className="text-[#67c23a] text-[13px] hover:underline"><CheckCircle className="h-3.5 w-3.5 inline mr-0.5"/>通过</button><button onClick={()=>audit(i.id,'REJECTED')} className="text-[#f56c6c] text-[13px] hover:underline ml-2"><XCircle className="h-3.5 w-3.5 inline mr-0.5"/>拒绝</button></div>:<span className="text-[13px] text-muted-foreground">-</span>}</ErpTd></ErpTr>))}
