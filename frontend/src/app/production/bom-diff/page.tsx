...
interface Item { id:string;code:string;name:string;version:string|null;materialName:string|null;quantity:string;approvalStatus:string;createdAt:string; }
...
export default function BomDiffPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    <div className="flex items-center justify-between px-4 h-14 border-b border-border">
      <span className="text-sm font-medium text-foreground">BOM差异分析</span>
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh>BOM编码</ErpTh><ErpTh>名称</ErpTh><ErpTh>版本</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>创建时间</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><ErpLink>{i.code||'-'}</ErpLink></ErpTd><ErpTd>{i.name||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.version||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd></ErpTr>))}
