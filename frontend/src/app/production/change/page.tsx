...
interface Item {
  ...
  businessStatus:string; remark:string|null; createdAt:string; createdBy?:string|null;
}
...
export default function ProductionChangePage() {
  ...
  const wf=async(id:string,action:'submit'|'withdraw')=>{
    try{await api.put(`/production-orders/${id}/${action}`);fetch();}
    catch(e:any){toast(e.response?.data?.message||'操作失败','error');}
  };
  const tryDel=(item:Item)=>{
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-lg border border-[#dcdfe6] bg-white">
      {/* Toolbar: 新增|修改|删除|提交|撤回|流程查看|打印 */}
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-[#ebeef5] bg-white px-4 py-3">
        <div className="flex items-center gap-1.5">
          <Button variant="secondary" size="sm" className="gap-1" onClick={()=>router.push('/production/change/create')}><Plus className="h-3.5 w-3.5"/>新增</Button>
          <Button variant="outline" size="sm" className="gap-1 text-[#67c23a]" disabled={!single}
            onClick={()=>selItem&&router.push(`/production/change/${selItem.id}/edit`)}><Pencil className="h-3.5 w-3.5"/>修改</Button>
          <Button variant="outline" size="sm" className="gap-1 text-[#f56c6c]" disabled={!single}
            onClick={()=>{if(selItem)tryDel(selItem);}}><Trash2 className="h-3.5 w-3.5"/>删除</Button>
          <DropdownMenu>
      ...
      <div className="shrink-0 border-b border-[#ebeef5] bg-[#fafafa] px-4 py-3">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <FI label="变更编号" v={s.code} onChange={v=>setS({...s,code:v})} ph="变更编号"/>
          <FI label="变更名称" v={s.name} onChange={v=>setS({...s,name:v})} ph="变更名称"/>
          ...
          <ErpTbody>
            {items.map(i=>{
              ...
              return (<ErpTr key={i.id}>
                ...
                <ErpTd><ErpLink onClick={()=>router.push(`/production/change/${i.id}/edit`)}>{i.orderNo}</ErpLink></ErpTd>
                <ErpTd>{i.orderName||'-'}</ErpTd>
                <ErpTd className="text-[#606266]">默认企业</ErpTd>
                <ErpTd className="text-[#606266]">{i.departmentName||'-'}</ErpTd>
                <ErpTd>{i.createdBy||'测试用户'}</ErpTd>
                ...
                <ErpTd className="sticky right-0 z-10 bg-white">
                  <ErpAction>
                    <ErpActionBtn onClick={()=>router.push(`/production/change/${i.id}/edit`)}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>
                    <ErpActionBtn danger onClick={()=>tryDel(i)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn>
