...
interface Item { id:string;orderNo:string;orderName:string;materialName:string|null;quantity:string;departmentName:string|null;approvalStatus:string;businessStatus:string;createdAt:string; }

export default function IssueQueryPage() {
  ...
  const fetch=useCallback(async()=>{
    const p:any={page:pg,pageSize:ps}; if(s.code)p.code=s.code; if(s.name)p.name=s.name; if(s.status)p.status=s.status; p.biz='ISSUING';
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh>生产订单号</ErpTh><ErpTh>订单名称</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>部门</ErpTh><ErpTh>业务状态</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>创建时间</ErpTh></ErpThead><ErpTbody>
