...
interface Item { id:string;code:string;name:string;processType:string|null;workCenter:string|null;standardTime:string|null;status:string;createdAt:string; }
...
export default function ProcessPage() {
  ...
  const fetch=useCallback(async()=>{
    ...
    const {data}=await api.get('/processes',{params:p}); setItems(data.items); setTotal(data.total);
  },[pg,ps,s]); useEffect(()=>{fetch();},[fetch]);
  const doDel=async()=>{if(!del)return;try{await api.delete(`/processes/${del}`);setDel(null);fetch();}catch(e:any){toast(e.response?.data?.message||'删除失败','error');}};
  return (<TooltipProvider><ErpListPage>
    <ErpToolbar addHref="/production/process/create" editDisabled={true} onDelete={()=>toast('请先勾选数据','info')} onReset={()=>setS({code:'',name:'',status:''})} onSearch={fetch} showExport onExport={()=>{}}/>
    <ErpSearchFields>
      <ErpSearchField label="工序编码"><Input className="w-[140px] h-9 rounded-md border border-border bg-background px-3 text-[13px]" value={s.code} onChange={e=>setS({...s,code:e.target.value})}/></ErpSearchField>
      <ErpSearchField label="工序名称"><Input className="w-[140px] h-9 rounded-md border border-border bg-background px-3 text-[13px]" value={s.name} onChange={e=>setS({...s,name:e.target.value})}/></ErpSearchField>
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh className="w-10"><Checkbox/></ErpTh><ErpTh>工序编码</ErpTh><ErpTh>工序名称</ErpTh><ErpTh>工序类型</ErpTh><ErpTh>工作中心</ErpTh><ErpTh>标准工时</ErpTh><ErpTh>状态</ErpTh><ErpTh>创建时间</ErpTh><ErpTh>操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox/></ErpTd><ErpTd><ErpLink>{i.code}</ErpLink></ErpTd><ErpTd>{i.name}</ErpTd><ErpTd className="text-[#909399]">{i.processType||'-'}</ErpTd><ErpTd className="text-[#909399]">{i.workCenter||'-'}</ErpTd><ErpTd>{i.standardTime||'-'}</ErpTd><ErpTd><ErpStatus active={i.status==='ACTIVE'}/></ErpTd><ErpTd className="text-[#909399]">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction><ErpActionBtn onClick={()=>router.push(`/production/process/${i.id}/edit`)}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn><ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
