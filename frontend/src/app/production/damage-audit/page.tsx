...
const pushPlan=async(i:Item)=>{await api.post('/purchase-plans',{orderNo:'PLAN-'+Date.now(),orderName:'补采购-'+i.materialName,supplierName:'',materialName:i.materialName,quantity:i.quantity,requiredDate:new Date().toISOString().split('T')[0]});alert('已生成采购计划');fetch();};
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><ErpLink>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.orderName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.departmentName||'-'}</ErpTd><ErpTd><ErpApproval status={i.businessStatus} labels={{PENDING_ISSUE:'待领料',IN_PRODUCTION:'生产中',PENDING_STOCK:'待入库',COMPLETED:'已完成'}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd className="text-muted-foreground max-w-[120px] truncate">{i.remark||'-'}</ErpTd><ErpTd>{i.approvalStatus==='SUBMITTED'?<div className="flex gap-1"><button onClick={()=>pushPlan(i)} className="text-[#e6a23c] text-[13px] hover:underline mr-2"><GitBranch className="h-3.5 w-3.5 inline mr-0.5"/>下推采购</button><button onClick={()=>audit(i.id,'APPROVED')} className="text-[#67c23a] text-[13px] hover:underline"><CheckCircle className="h-3.5 w-3.5 inline mr-0.5"/>通过</button><button onClick={()=>audit(i.id,'REJECTED')} className="text-[#f56c6c] text-[13px] hover:underline ml-2"><XCircle className="h-3.5 w-3.5 inline mr-0.5"/>拒绝</button></div>:<span className="text-[13px] text-muted-foreground">-</span>}</ErpTd></ErpTr>))}
