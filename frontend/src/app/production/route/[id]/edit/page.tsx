...
export default function RouteEdit(){const router=useRouter();const {id}=useParams();const [f,setF]=useState({code:'',name:'',version:'',materialName:'',quantity:''} as any);
useEffect(()=>{api.get('/boms/'+id).then(r=>setF(r.data));},[id]);
const save=async()=>{await api.put('/boms/'+id,f);router.push('/production/route');};
return(<FormLayout title="修改工艺路线" onSave={save} sections={[{id:'b',title:'路由信息'}]} activeSection="b"><FormSection id="b" title="路由信息"><FormGrid>
<FormField label="路由编码" required><Input className={FI} value={f.code} onChange={e=>setF({...f,code:e.target.value})}/></FormField>
<FormField label="路由名称" required><Input className={FI} value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></FormField>
