...
export default function PurchaseReturnEdit(){const router=useRouter();const {id}=useParams();const [f,setF]=useState({orderNo:'',orderName:'',customerName:'',totalAmount:''} as any);
useEffect(()=>{api.get('/sales-returns/'+id).then(r=>setF(r.data));},[id]);
const save=async()=>{await api.put('/sales-returns/'+id,f);router.push('/purchase/return');};
