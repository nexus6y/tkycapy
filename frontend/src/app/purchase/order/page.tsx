...
const BS_LABELS:Record<string,string> = { PENDING_RECEIPT:'待入库', PARTIAL_RECEIPT:'部分入库', FULLY_RECEIVED:'入库完成', INSPECTING:'质检中' };
const BS_COLOR = (s:string) => s==='FULLY_RECEIVED'?'text-[#67c23a]':s==='INSPECTING'?'text-[#409eff]':s==='PARTIAL_RECEIPT'?'text-[#e6a23c]':'text-muted-foreground';
...
export default function PurchaseOrderPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="flex items-center gap-4 px-4 py-2.5 border-b border-border bg-muted/30 flex-wrap">
      ...
      <F label="业务"><Select value={s.bizStatus} onValueChange={(v:any)=>setS({...s,bizStatus:v==='ALL'?'':v})}><SelectTrigger className="w-[100px] h-9 rounded-md border border-border bg-background px-3 text-[13px]"><SelectValue placeholder="全部"/></SelectTrigger><SelectContent><SelectItem value="ALL">全部</SelectItem><SelectItem value="PENDING_RECEIPT">待入库</SelectItem><SelectItem value="PARTIAL_RECEIPT">部分入库</SelectItem><SelectItem value="INSPECTING">质检中</SelectItem><SelectItem value="FULLY_RECEIVED">入库完成</SelectItem></SelectContent></Select></F>
