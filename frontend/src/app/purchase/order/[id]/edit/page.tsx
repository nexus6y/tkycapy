'use client';import { useState, useEffect } from 'react';import { useRouter, useParams } from 'next/navigation';import api from '@/lib/api';import { Input } from '@/components/ui/input';import { FormLayout,FormSection,FormGrid,FormField } from '@/components/form/form-layout';import { LinesEditor, LineItem } from '@/components/ui/lines-editor';
const FI='h-9 rounded-md border border-border bg-background px-3 text-[13px] w-full';
export default function PurchaseOrderEdit(){const router=useRouter();const {id}=useParams();const [f,setF]=useState({orderNo:'',orderName:'',supplierName:'',purchaser:'',orderType:'',totalAmount:'',expectedDeliveryDate:'',remark:''} as any);
const [lines,setLines]=useState<LineItem[]>([]);
useEffect(()=>{api.get('/purchase-orders/'+id).then(r=>{const d=r.data;setF({orderNo:d.orderNo||'',orderName:d.orderName||'',supplierName:d.supplierName||'',purchaser:d.purchaser||'',orderType:d.purchaseType||'',totalAmount:d.totalAmount||'',expectedDeliveryDate:d.expectedDeliveryDate?d.expectedDeliveryDate.slice(0,10):'',remark:d.remark||''});if(d.lines)setLines(d.lines);});},[id]);
const save=async()=>{await api.put('/purchase-orders/'+id,{...f,lines});router.push('/purchase/order');};
