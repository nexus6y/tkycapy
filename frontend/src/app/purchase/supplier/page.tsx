...
export default function SupplierPage() {
  ...
  const doDel = async () => {
    if (!del) return;
    try {
      await api.delete(`/suppliers/${del}`);
      setDel(null);
      fetch();
    } catch (e: any) {
  ...
  return (
    <TooltipProvider>
      <ErpListPage>
        <ErpToolbar
          addHref="/purchase/supplier/create"
            ...
            <ErpTbody>
              {items.map(i => (
                <ErpTr key={i.id}>
                  ...
                  <ErpTd><ErpLink onClick={() => router.push(`/purchase/supplier/${i.id}/edit`)}>{i.code}</ErpLink></ErpTd>
                  <ErpTd>{i.name}</ErpTd>
                  <ErpTd className="text-muted-foreground">{i.contactPerson || '-'}</ErpTd>
                  <ErpTd className="text-muted-foreground">{i.contactPhone || '-'}</ErpTd>
                  <ErpTd>{i.creditLevel || '-'}</ErpTd>
                  <ErpTd className="text-muted-foreground">{i.taxId || '-'}</ErpTd>
                  <ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd>
                  <ErpTd>
                    <ErpAction>
                      <ErpActionBtn onClick={() => router.push(`/purchase/supplier/${i.id}/edit`)}>
