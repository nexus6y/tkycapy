...
export default function SupplierPage() {
  ...
  const doDel=async()=>{if(!del)return;await api.delete(`/suppliers/${del}`);setDel(null);fetch();};
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox/></ErpTd><ErpTd><ErpStatus active={i.status==='ACTIVE'}/></ErpTd><ErpTd><ErpLink>{i.code}</ErpLink></ErpTd><ErpTd>{i.name}</ErpTd><ErpTd className="text-muted-foreground">{i.contactPerson||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.contactPhone||'-'}</ErpTd><ErpTd>{i.creditLevel||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.taxId||'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction><ErpActionBtn><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn><ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
