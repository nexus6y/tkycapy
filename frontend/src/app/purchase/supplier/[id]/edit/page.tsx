...
const save=async()=>{try{await api.put('/suppliers/'+id,f);router.push('/purchase/supplier');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
...
<FormField label="状态"><RadioGroup value={f.status} onValueChange={v=>setF({...f,status:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="ACTIVE" id="se-a"/><label htmlFor="se-a" className="text-[13px]">启用</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="INACTIVE" id="se-i"/><label htmlFor="se-i" className="text-[13px]">停用</label></div></RadioGroup></FormField>
