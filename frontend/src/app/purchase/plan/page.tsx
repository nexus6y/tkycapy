...
export default function PurchasePlanPage() {
  ...
  const doDel=async()=>{if(!del)return;try{await api.delete(`/purchase-plans/${del}`);setDel(null);fetch();}catch(e:any){toast(e.response?.data?.message||'删除失败','error');}};
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    <div className="flex items-center justify-between px-4 h-14 border-b border-border">
      <div className="flex items-center gap-1">
        <Button variant="secondary" size="sm" onClick={()=>router.push('/purchase/plan/create')}><Plus className="h-3.5 w-3.5"/>新增</Button>
    ...
    <div className="flex items-center gap-4 px-4 py-2.5 border-b border-border bg-muted/30 flex-wrap">
      ...
      <F label="计划编号"><Input className="w-[140px] h-9 rounded-md border border-border bg-background px-3 text-[13px]" value={s.code} onChange={e=>setS({...s,code:e.target.value})}/></F>
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh className="w-10"><Checkbox checked={items.length>0&&sel.size===items.length} onCheckedChange={(v:boolean)=>toggleAll(v)}/></ErpTh><ErpTh>计划编号</ErpTh><ErpTh>计划名称</ErpTh><ErpTh>客户/供应商</ErpTh><ErpTh>金额</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>创建时间</ErpTh><ErpTh>操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox checked={sel.has(i.id)} onCheckedChange={(v:boolean)=>{const n=new Set(sel);v?n.add(i.id):n.delete(i.id);setSel(n);}}/></ErpTd><ErpTd><ErpLink onClick={()=>router.push(`/purchase/plan/${i.id}/edit`)}>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.orderName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.supplierName||'-'}</ErpTd><ErpTd>{i.totalAmount?Number(i.totalAmount).toLocaleString():'-'}</ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction>{i.approvalStatus==='DRAFT'&&<ErpActionBtn onClick={async()=>{await api.put('/purchase-plans/'+i.id+'/submit');fetch();}}>提交</ErpActionBtn>}{i.approvalStatus==='SUBMITTED'&&<ErpActionBtn onClick={async()=>{await api.put('/purchase-plans/'+i.id+'/withdraw');fetch();}}>撤回</ErpActionBtn>}{i.approvalStatus==='SUBMITTED'&&<button onClick={()=>{if(window.confirm('审批通过后将自动生成采购订单。确定继续？')){api.put(`/purchase-plans/${i.id}/approve`).then(()=>{fetch();toast('已审批，采购订单已生成','success');}).catch((e:any)=>toast(e.response?.data?.message||'审批失败','error'));}}} className="text-green-600 text-[13px] hover:underline">审批/下推采购订单</button>}{i.approvalStatus==='APPROVED'&&<span className="text-[12px] text-muted-foreground">已生成采购订单</span>}<ErpActionBtn onClick={()=>router.push(`/purchase/plan/${i.id}/edit`)}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn><ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
