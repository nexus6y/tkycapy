'use client';import { useState, useEffect } from 'react';import { useRouter, useParams } from 'next/navigation';import api from '@/lib/api';import { Input } from '@/components/ui/input';import { Select,SelectContent,SelectItem,SelectTrigger,SelectValue } from '@/components/ui/select';import { FormLayout,FormSection,FormGrid,FormField } from '@/components/form/form-layout';const FI='h-9 rounded-md border border-border bg-background px-3 text-[13px] w-full';
export default function PurchasePlanEdit(){const router=useRouter();const {id}=useParams();const [suppliers,setSuppliers]=useState<any[]>([]);const [f,setF]=useState({orderNo:'',orderName:'',supplierId:'',supplierName:'',materialName:'',quantity:'',requiredDate:''} as any);
useEffect(()=>{api.get('/suppliers',{params:{pageSize:999}}).then(r=>setSuppliers(r.data.items));api.get('/purchase-plans/'+id).then(r=>{const d=r.data;setF({...d,requiredDate:d.requiredDate?d.requiredDate.split('T')[0]:''});});},[id]);
