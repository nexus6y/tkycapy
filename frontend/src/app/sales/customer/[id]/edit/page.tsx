...
const save=async()=>{try{await api.put('/customers/'+id,f);router.push('/sales/customer');}catch(e:any){toast(e.response?.data?.message||'保存失败','error');}};
...
<FormField label="状态"><RadioGroup value={f.status} onValueChange={(v:any)=>setF({...f,status:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="ACTIVE" id="ce-a"/><label htmlFor="ce-a" className="text-[13px]">启用</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="INACTIVE" id="ce-i"/><label htmlFor="ce-i" className="text-[13px]">停用</label></div></RadioGroup></FormField>
