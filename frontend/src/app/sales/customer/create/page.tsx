...
export default function CustomerCreatePage() {
  ...
  const save=async()=>{try{await api.post('/customers',f);router.push('/sales/customer');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
  return (<FormLayout title="新增客户" onSave={save} sections={[{id:'b',title:'基本信息'},{id:'c',title:'联系方式'}]} activeSection="b">
    <FormSection id="b" title="基本信息"><FormGrid>
      ...
      <FormField label="状态"><RadioGroup value={f.status} onValueChange={v=>setF({...f,status:v})} className="flex gap-4 pt-1.5"><div className="flex items-center gap-1.5"><RadioGroupItem value="ACTIVE" id="cs-a"/><label htmlFor="cs-a" className="text-[13px]">启用</label></div><div className="flex items-center gap-1.5"><RadioGroupItem value="INACTIVE" id="cs-i"/><label htmlFor="cs-i" className="text-[13px]">停用</label></div></RadioGroup></FormField>
