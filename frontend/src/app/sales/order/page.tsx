...
interface Item { id:string;orderNo:string;orderName:string;customerName:string|null;projectName:string|null;orderType:string|null;totalAmount:string|null;approvalStatus:string;businessStatus:string;createdAt:string; }
...
export default function SalesOrderPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="flex items-center gap-4 px-4 py-2.5 border-b border-border bg-muted/30 flex-wrap">
      <F label="审批"><Select value={s.status} onValueChange={v=>setS({...s,status:v==='ALL'?'':v})}><SelectTrigger className="w-[90px] h-9 rounded-md border border-border bg-background px-3 text-[13px]"><SelectValue placeholder="全部"/></SelectTrigger><SelectContent><SelectItem value="ALL">全部</SelectItem><SelectItem value="DRAFT">草稿</SelectItem><SelectItem value="SUBMITTED">已提交</SelectItem><SelectItem value="APPROVED">已通过</SelectItem></SelectContent></Select></F>
      <F label="业务"><Select value={s.bizStatus} onValueChange={v=>setS({...s,bizStatus:v==='ALL'?'':v})}><SelectTrigger className="w-[100px] h-9 rounded-md border border-border bg-background px-3 text-[13px]"><SelectValue placeholder="全部"/></SelectTrigger><SelectContent><SelectItem value="ALL">全部</SelectItem><SelectItem value="PENDING_SHIP">待出货</SelectItem><SelectItem value="PARTIAL_SHIP">部分出货</SelectItem><SelectItem value="FULLY_SHIPPED">出货完成</SelectItem></SelectContent></Select></F>
      <F label="订单号"><Input className="w-[130px] h-9 rounded-md border border-border bg-background px-3 text-[13px]" value={s.code} onChange={e=>setS({...s,code:e.target.value})}/></F>
      <F label="订单名称"><Input className="w-[130px] h-9 rounded-md border border-border bg-background px-3 text-[13px]" value={s.name} onChange={e=>setS({...s,name:e.target.value})}/></F>
    </div>
    ...
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><span className="text-[12px] text-muted-foreground">{BS[i.businessStatus]||i.businessStatus}</span></ErpTd><ErpTd><ErpLink onClick={()=>router.push('/sales/order/'+i.id+'/edit')}>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.orderName}</ErpTd><ErpTd className="text-muted-foreground">{i.customerName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.projectName||'-'}</ErpTd><ErpTd>{i.totalAmount?Number(i.totalAmount).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction><ErpActionBtn onClick={()=>router.push('/sales/order/'+i.id+'/edit')}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>{i.approvalStatus==='DRAFT'&&<button onClick={()=>{api.put(`/sales-orders/${i.id}/submit`).then(fetch);}} className="text-primary text-[13px]">提交</button>}{i.approvalStatus==='SUBMITTED'&&<button onClick={()=>{api.put(`/sales-orders/${i.id}/approve`).then(fetch);}} className="text-green-600 text-[13px]">通过</button>}{i.approvalStatus==='SUBMITTED'&&<button onClick={()=>{api.put(`/sales-orders/${i.id}/withdraw`).then(fetch);}} className="text-orange-500 text-[13px]">撤回</button>}<ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
