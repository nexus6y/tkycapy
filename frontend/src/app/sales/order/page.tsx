...
export default function SalesOrderPage() {
  ...
  const doDel=async()=>{if(!del)return;try{await api.delete(`/sales-orders/${del}`);setDel(null);fetch();}catch(e:any){toast(e.response?.data?.message||'删除失败','error');}};
    ...
    <div className="flex items-center justify-between px-4 h-14 border-b border-border">
      ...
      <div className="flex items-center gap-1"><Button variant="ghost" size="sm" onClick={()=>setS({code:'',name:'',status:'',bizStatus:''})}>重置</Button><Button variant={detailMode?"secondary":"outline"} size="sm" onClick={()=>setDetailMode(!detailMode)}>主单+明细</Button><Button variant="default" size="sm" onClick={fetch}><Search className="h-3.5 w-3.5 mr-1"/>搜索</Button></div>
    </div>
