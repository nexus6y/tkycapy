...
export default function SRCreate(){const router=useRouter();useEffect(()=>{api.get('/common/next-code',{params:{entity:'salesReturn'}}).then(r=>setF((prev:any)=>({...prev,returnNo:r.data.code})));},[]);const [f,setF]=useState({returnNo:'',shipmentNo:'',customerName:'',totalQuantity:'',totalAmount:'',returnReason:''});
...
<FormField label="退货单号" required><Input className={FI} value={f.returnNo} readOnly disabled/></FormField>
