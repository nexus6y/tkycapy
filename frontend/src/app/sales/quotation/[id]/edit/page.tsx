...
const save=async()=>{
  if(lines.length>0){f.totalAmount=String(lines.reduce((s,l)=>s+Number(l.amount||0)||(Number(l.quantity||0)*Number(l.unitPrice||0)),0));}
  await api.put('/quotations/'+id,{...f,lines});router.push('/sales/quotation');
};
...
<FormField label="金额"><Input type="number" className={FI} value={f.totalAmount||''} disabled/></FormField>
