...
interface Item { id:string;quotationNo:string;quotationName:string;customerName:string|null;departmentName:string|null;responsibleName:string|null;validUntil:string|null;totalAmount:string|null;markResult:string;approvalStatus:string;createdAt:string; }
...
export default function QuotationPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="min-h-0"><ErpTable><ErpThead><ErpTh className="w-[48px]"><Checkbox checked={items.length>0&&selected.size===items.length} onCheckedChange={(v:boolean)=>toggleAll(v)}/></ErpTh><ErpTh className="w-[100px]">审批状态</ErpTh><ErpTh className="w-[170px]">报价单号</ErpTh><ErpTh className="w-[200px]">报价名称</ErpTh><ErpTh className="w-[180px]">客户</ErpTh><ErpTh className="w-[130px]">报价部门</ErpTh><ErpTh className="w-[100px]">负责人</ErpTh><ErpTh className="w-[120px]">有效期至</ErpTh><ErpTh className="w-[140px]">金额</ErpTh><ErpTh className="w-[160px]">创建时间</ErpTh><ErpTh className="w-[200px] sticky right-0 bg-[#f5f7fa] z-10">操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox checked={selected.has(i.id)} onCheckedChange={(v:boolean)=>{const n=new Set(selected);v?n.add(i.id):n.delete(i.id);setSelected(n);}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><ErpLink onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}>{i.quotationNo}</ErpLink></ErpTd><ErpTd>{i.quotationName}</ErpTd><ErpTd className="text-muted-foreground">{i.customerName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.departmentName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.responsibleName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.validUntil?new Date(i.validUntil).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd>{i.totalAmount?Number(i.totalAmount).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd className="sticky right-0 bg-white z-10"><ErpAction><ErpActionBtn onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>{i.approvalStatus==='DRAFT'&&<ErpActionBtn onClick={()=>{api.put(`/quotations/${i.id}/submit`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'失败','error'));}}>提交</ErpActionBtn>}{i.approvalStatus==='SUBMITTED'&&<ErpActionBtn onClick={()=>{api.put(`/quotations/${i.id}/approve`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'失败','error'));}}>通过</ErpActionBtn>}{i.approvalStatus==='APPROVED'&&i.markResult!=='WON'&&<ErpActionBtn onClick={()=>{if(window.confirm('确定下推分劈单？将从报价明细生成分劈单。')){api.post(`/quotations/${i.id}/generate-pre-order`).then(()=>{fetch();toast('分劈单已生成','success');}).catch((e:any)=>toast(e.response?.data?.message||'下推失败','error'));}}}>下推分劈单</ErpActionBtn>}{i.markResult==='WON'&&<span className="text-[12px] text-muted-foreground">已下推</span>}<ErpActionBtn danger onClick={()=>setDelId(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
