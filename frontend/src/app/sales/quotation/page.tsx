...
export default function QuotationPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh className="w-10"><Checkbox checked={items.length>0&&selected.size===items.length} onCheckedChange={(v:boolean)=>toggleAll(v)}/></ErpTh><ErpTh>审批状态</ErpTh><ErpTh>报价单号</ErpTh><ErpTh>报价名称</ErpTh><ErpTh>客户</ErpTh><ErpTh>报价部门</ErpTh><ErpTh>负责人</ErpTh><ErpTh>有效期至</ErpTh><ErpTh>金额</ErpTh><ErpTh>创建时间</ErpTh><ErpTh>操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox checked={selected.has(i.id)} onCheckedChange={(v:boolean)=>{const n=new Set(selected);v?n.add(i.id):n.delete(i.id);setSelected(n);}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><ErpLink onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}>{i.quotationNo}</ErpLink></ErpTd><ErpTd>{i.quotationName}</ErpTd><ErpTd className="text-muted-foreground">{i.customerName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.departmentName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.responsibleName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.validUntil?new Date(i.validUntil).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd>{i.totalAmount?Number(i.totalAmount).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction><ErpActionBtn onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>{i.approvalStatus==='DRAFT'&&<button onClick={()=>{api.put(`/quotations/${i.id}/submit`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'失败','error'));}} className="text-primary text-[13px] hover:underline">提交</button>}{i.approvalStatus==='SUBMITTED'&&<button onClick={()=>{api.put(`/quotations/${i.id}/approve`).then(fetch).catch((e:any)=>toast(e.response?.data?.message||'失败','error'));}} className="text-green-600 text-[13px] hover:underline">通过</button>}{i.approvalStatus==='APPROVED'&&i.markResult!=='WON'&&<button onClick={()=>{if(window.confirm('确定下推分劈单？将从报价明细生成分劈单。')){api.post(`/quotations/${i.id}/generate-pre-order`).then(()=>{fetch();toast('分劈单已生成','success');}).catch((e:any)=>toast(e.response?.data?.message||'下推失败','error'));}}} className="text-blue-600 text-[13px] hover:underline">下推分劈单</button>}{i.markResult==='WON'&&<span className="text-[12px] text-muted-foreground">已下推</span>}<ErpActionBtn danger onClick={()=>setDelId(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
