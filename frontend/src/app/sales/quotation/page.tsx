...
export default function QuotationPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox checked={selected.has(i.id)} onCheckedChange={(v:boolean)=>{const n=new Set(selected);v?n.add(i.id):n.delete(i.id);setSelected(n);}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><ErpLink onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}>{i.quotationNo}</ErpLink></ErpTd><ErpTd>{i.quotationName}</ErpTd><ErpTd className="text-muted-foreground">{i.customerName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.departmentName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.responsibleName||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.validUntil?new Date(i.validUntil).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd>{i.totalAmount?Number(i.totalAmount).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{new Date(i.createdAt).toLocaleDateString('zh-CN')}</ErpTd><ErpTd><ErpAction>{i.approvalStatus==='DRAFT'&&<ErpActionBtn onClick={async()=>{await api.put('/quotations/'+i.id+'/submit');fetch();}}>提交</ErpActionBtn>}{i.approvalStatus==='SUBMITTED'&&<ErpActionBtn onClick={async()=>{await api.put('/quotations/'+i.id+'/approve');fetch();}}>通过</ErpActionBtn>}<ErpActionBtn onClick={()=>router.push('/sales/quotation/'+i.id+'/edit')}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn>{i.approvalStatus==='DRAFT'&&<button onClick={()=>{api.put(`/quotations/${i.id}/submit`).then(fetch);}} className="text-primary text-[13px]">提交</button>}<ErpActionBtn danger onClick={()=>setDelId(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
