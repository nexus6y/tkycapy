...
export default function WarehouseCreatePage() {
  ...
  const save=async()=>{if(!f.name)return toast('请填写仓库名称','error');await api.post('/warehouses',f);router.push('/warehouse/warehouse');};
  return (<FormLayout title="新增仓库" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b">
    <FormSection id="b" title="基本信息"><FormGrid>
      <FormField label="仓库编码"><Input className={FI} value={f.code} readOnly disabled/></FormField>
