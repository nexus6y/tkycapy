...
interface Item { id:string;orderNo:string;materialName:string|null;quantity:string;scrapReason:string|null;disposalMethod:string|null;approvalStatus:string;scrapDate:string; }

export default function ScrapDisposePage() {
  ...
  const fetch=useCallback(async()=>{
    ...
    const {data}=await api.get('/scrap-orders',{params:p}); setItems(data.items); setTotal(data.total);
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh>报废单号</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>报废原因</ErpTh><ErpTh>处置方式</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>日期</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><ErpLink>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.scrapReason||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.disposalMethod||'-'}</ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd className="text-muted-foreground">{i.scrapDate?new Date(i.scrapDate).toLocaleDateString('zh-CN'):'-'}</ErpTd></ErpTr>))}
