'use client';import { useState, useEffect } from 'react';import { useRouter, useParams } from 'next/navigation';import api from '@/lib/api';import { Input } from '@/components/ui/input';import { Select,SelectContent,SelectItem,SelectTrigger,SelectValue } from '@/components/ui/select';import { RadioGroup,RadioGroupItem } from '@/components/ui/radio-group';import { toast } from '@/components/ui/toast';
...
export default function LocationEdit(){const router=useRouter();const {id}=useParams();const [shelves,setShelves]=useState<any[]>([]);const [f,setF]=useState({code:'',name:'',type:'',usageStatus:'',shelfId:'',shelfName:'',layer:'',col:'',zoneName:'',warehouseName:'',sortOrder:0,status:'ACTIVE'});
useEffect(()=>{api.get('/shelves',{params:{pageSize:999}}).then(r=>setShelves(r.data.items));api.get('/locations/'+id).then(r=>setF(r.data));},[id]);
const save=async()=>{try{await api.put('/locations/'+id,f);router.push('/warehouse/location');}catch(e:any){toast(e.response?.data?.message||'保存失败','error');}};
return(<FormLayout title="修改货位" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b"><FormSection id="b" title="基本信息"><FormGrid>
<FormField label="货位编码" required><Input className={FI} value={f.code} onChange={e=>setF({...f,code:e.target.value})}/></FormField>
