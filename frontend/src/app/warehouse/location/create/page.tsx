...
useEffect(()=>{api.get('/shelves',{params:{pageSize:999}}).then(r=>setShelves(r.data.items));},[]);
const save=async()=>{try{await api.post('/locations',f);router.push('/warehouse/location');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
