...
interface Item { id:string;orderNo:string;type:string;materialName:string|null;quantity:string;fromWarehouse:string|null;toWarehouse:string|null;approvalStatus:string;transferDate:string; }
...
export default function TransferInPage() {
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh>调入单号</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>调出仓库</ErpTh><ErpTh>调入仓库</ErpTh><ErpTh>审批状态</ErpTh><ErpTh>日期</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><ErpLink>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.fromWarehouse||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.toWarehouse||'-'}</ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd className="text-muted-foreground">{i.transferDate?new Date(i.transferDate).toLocaleDateString('zh-CN'):'-'}</ErpTd></ErpTr>))}
