...
interface Item { id:string;orderNo:string;type:string;materialName:string|null;quantity:string;fromWarehouse:string|null;toWarehouse:string|null;approvalStatus:string;businessStatus:string;transferDate:string;createdAt:string; }
...
export default function TransferOutPage() {
  ...
  const fetch=useCallback(async()=>{
    ...
    const {data}=await api.get('/transfer-orders',{params:{...p,type:'OUT'}}); setItems(data.items); setTotal(data.total);
  },[pg,ps,s]); useEffect(()=>{fetch();},[fetch]);

  const doDel=async()=>{if(!del)return;try{await api.delete(`/transfer-orders/${del}`);setDel(null);fetch();}catch(e:any){toast(e.response?.data?.message||'删除失败','error');}};
  ...
  return (<TooltipProvider><div className="bg-background rounded-lg border shadow-sm">
    <div className="flex items-center justify-between px-4 h-14 border-b border-border">
      <div className="flex items-center gap-1">
        <Button variant="secondary" size="sm" onClick={()=>router.push('/warehouse/transfer-out/create')}><Plus className="h-3.5 w-3.5"/>新增</Button>
    ...
    <div className="overflow-auto"><ErpTable><ErpThead><ErpTh className="w-10"><Checkbox checked={items.length>0&&sel.size===items.length} onCheckedChange={(v:boolean)=>toggleAll(v)}/></ErpTh><ErpTh>审批状态</ErpTh><ErpTh>调出单号</ErpTh><ErpTh>物料</ErpTh><ErpTh>数量</ErpTh><ErpTh>调出仓库</ErpTh><ErpTh>调入仓库</ErpTh><ErpTh>调拨日期</ErpTh><ErpTh>操作</ErpTh></ErpThead><ErpTbody>
    {items.map(i=>(<ErpTr key={i.id}><ErpTd><Checkbox checked={sel.has(i.id)} onCheckedChange={(v:boolean)=>{const n=new Set(sel);v?n.add(i.id):n.delete(i.id);setSel(n);}}/></ErpTd><ErpTd><ErpApproval status={i.approvalStatus}/></ErpTd><ErpTd><ErpLink onClick={()=>router.push('/warehouse/transfer-out/'+i.id+'/edit')}>{i.orderNo}</ErpLink></ErpTd><ErpTd>{i.materialName||'-'}</ErpTd><ErpTd>{i.quantity?Number(i.quantity).toLocaleString():'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.fromWarehouse||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.toWarehouse||'-'}</ErpTd><ErpTd className="text-muted-foreground">{i.transferDate?new Date(i.transferDate).toLocaleDateString('zh-CN'):'-'}</ErpTd><ErpTd><ErpAction>{i.approvalStatus==='DRAFT'&&<ErpActionBtn onClick={async()=>{await api.put('/transfer-orders/'+i.id+'/submit');fetch();}}>提交</ErpActionBtn>}{i.approvalStatus==='SUBMITTED'&&<ErpActionBtn onClick={async()=>{await api.put('/transfer-orders/'+i.id+'/approve');fetch();}}>通过</ErpActionBtn>}<ErpActionBtn onClick={()=>router.push('/warehouse/transfer-out/'+i.id+'/edit')}><Pencil className="h-3.5 w-3.5"/>修改</ErpActionBtn><ErpActionBtn danger onClick={()=>setDel(i.id)}><Trash2 className="h-3.5 w-3.5"/>删除</ErpActionBtn></ErpAction></ErpTd></ErpTr>))}
