...
const save=async()=>{try{await api.post('/warehouses',{...f,status:f.status==='ACTIVE'?'ACTIVE':'INACTIVE'});router.push('/warehouse/area');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
return(<FormLayout title="新增地区" onSave={save} sections={[{id:'b',title:'基本信息'}]} activeSection="b"><FormSection id="b" title="基本信息"><FormGrid>
<FormField label="地区编码" required><Input className={FI} value={f.code} onChange={e=>setF({...f,code:e.target.value})} placeholder="请输入地区编码"/></FormField>
