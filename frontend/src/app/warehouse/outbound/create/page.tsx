...
export default function OCreate(){const router=useRouter();const [f,setF]=useState({orderNo:'',materialName:'',specification:'',quantity:'',warehouseName:'',unitPrice:'',totalAmount:'',remark:''});
const save=async()=>{try{await api.post('/outbound-orders',f);router.push('/warehouse/outbound');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
return(<FormLayout title="新增出库单" onSave={save} sections={[{id:'b',title:'出库信息'}]} activeSection="b"><FormSection id="b" title="出库信息"><FormGrid>
<FormField label="出库单号" required><Input className={FI} value={f.orderNo} onChange={e=>setF({...f,orderNo:e.target.value})}/></FormField>
