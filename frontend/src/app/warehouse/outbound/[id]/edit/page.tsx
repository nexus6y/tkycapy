...
useEffect(()=>{api.get('/outbound-orders/'+id).then(r=>{setF(r.data);setL(false);});},[id]);
const save=async()=>{try{await api.put('/outbound-orders/'+id,f);router.push('/warehouse/outbound');}catch(e:any){alert(e.response?.data?.message||'保存失败');}};
