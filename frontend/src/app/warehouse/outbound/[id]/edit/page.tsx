...
api.get('/outbound-orders/'+id).then(r=>{setF({...r.data,materialId:r.data.materialId||'',zoneId:r.data.zoneId||''});setL(false);});
},[id]);
const label=(arr:any[],v:any,field='name')=>arr.find(x=>x.id===v)?.[field]||v;
const save=async()=>{const p:any={...f};['materialId','zoneId'].forEach(k=>delete p[k]);await api.put('/outbound-orders/'+id,p);router.push('/warehouse/outbound');};
if(l)return<div className="h-full flex items-center justify-center text-muted-foreground">加载中...</div>;
return(<FormLayout title={'编辑出库单：'+f.orderNo} onSave={save} sections={[{id:'b',title:'出库信息'}]} activeSection="b"><FormSection id="b" title="出库信息"><FormGrid>
