...
const onMaterialSelect=(id:string,m:any)=>{
  ...
  setF(prev=>({...prev,...fill,
    ...
    spec:fill.specification,unit:fill.unit,warehouseCode:fill.defaultWarehouseId||prev.warehouseCode,
  }));
...
<FormField label="物料">
  <EntitySelect entity="material" value={f.materialId}
    onChange={(id,m)=>{onMaterialSelect(id,m);}}/></FormField>
<FormField label="规格型号"><Input className={FI} value={f.spec} onChange={e=>setF({...f,spec:e.target.value})}/></FormField>
<FormField label="单位"><Input className={FI} value={f.unit} onChange={e=>setF({...f,unit:e.target.value})}/></FormField>
